...
// than 200, so a cached non-200 representation must be served in full rather
// than answered with 304.
func TestBugHuntIfModifiedSinceIgnoredForNon200CachedResponse(t *testing.T) {
	const lastModified = "Thu, 10 Sep 2026 03:58:45 GMT"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if second.Code != tt.status {
				t.Fatalf("expected If-Modified-Since to be ignored for cached %d, got status %d", tt.status, second.Code)
			}
